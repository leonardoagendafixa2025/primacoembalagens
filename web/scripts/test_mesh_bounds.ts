...
console.log('INDIVIDUAL MESH WORLD BOUNDS AT 0%:');
for (const p of tree.topology.panels) {
  ...
  if (pivot) {
    ...
    if (mesh) {
      ...
      console.log(p.id.padEnd(9), p.name.padEnd(16), 'X: [' + box.min.x.toFixed(1).padStart(6) + ', ' + box.max.x.toFixed(1).padStart(6) + ']', 'Z: [' + box.min.z.toFixed(1).padStart(6) + ', ' + box.max.z.toFixed(1).padStart(6) + ']', 'Y: [' + box.min.y.toFixed(1) + ', ' + box.max.y.toFixed(1) + ']');
    }
  }
}
