...
/**
 * FEFCO 0200 - Caixa Meia-Maleta Aberta / Gaveta (Half Slotted Container)
 * Portabilidade matemática 1:1 do PLMPackLib C# original (f_0200 / half_0200)
 * 
 * Componentes originais integrados:
 * 1. Fefco_0200 (Guid: 56747cac-bb08-4e27-9a20-9894f55fb0b4)
...
export const fefco0200: PackagingModel = {
  ...
  id: 'fefco_0200',
  code: 'FEFCO 0200',
  name: 'Caixa Meia-Maleta (Half Slotted Box)',
  category: 'FEFCO',
  description:
    'Modelo padrão internacional FEFCO 0200 / HSC do PLMPackLib com proporções paramétricas exatas e folgas de vinco C#.',
  defaultParams: {
  ...
  calculate(params: Record<string, number>): DielineResult {
    ...
    // Constantes e parâmetros padrão do fefco_0200 no PLMPackLib
    ...
    const H1_full = H + 2 * e;
    const H1 = H1_full / 2;
    // No FEFCO 0200 C# original (56747cac):
    // FL = Math.Floor((B + e) / 2)
    // FB = Math.Floor((L + e) / 2)
    const FL = Math.floor((B + e) / 2);
    const FB = Math.floor((L + e) / 2);
    ...
    // A chamada original C# do fefco_0200 executa com reflectionX=true:
